...
- Homelab managed with [OpenTofu](https://opentofu.org/), currently mainly for DNS configuration.
- A remote VPS on [Hetzner](https://www.hetzner.com/) runs [Pangolin](https://pangolin.net/) for authentication/authorization and tunneling to homelab services, providing isolation between internet-facing services and the homelab.
...
2. **Repo structure:** Keep OpenTofu (Hetzner provisioning) and NixOS config together in `homelab-infra`, since they're tightly coupled. Structure under `vps-pangolin/tofu/` and `vps-pangolin/nixos/`. Use a layout compatible with [Terragrunt](https://terragrunt.gruntwork.io/) from the start — `homelab-dns` and `homelab-infra` are candidates for future consolidation into a single repo once Terragrunt is introduced to manage per-unit state isolation.
3. **Provisioning workflow:** OpenTofu provisions the Hetzner VPS, then [nixos-anywhere](https://github.com/nix-community/nixos-anywhere) installs the declared NixOS config over SSH.
